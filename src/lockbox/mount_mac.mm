...
#import <lockbox/mount_webdav_interpose.h>
...
get_mount_device(std::string mount_point) {
    ...
    if (ret < 0) throw std::runtime_error("statfs failed");
    return b.f_mntfromname;
}

bool
MountDetails::is_still_mounted() const {
    // TODO: don't fail if mount point doesn't exist!
...
public:
    SystemGlobalMutex(std::string name) : _fd(-1), _name(name) {
        ...
        auto _unlock_mutex = lockbox::create_deferred(pthread_mutex_unlock, &_g_name_map_mutex);
...
public:
    ...
    RamDiskHandle() : _mapping((SharedStorage *) MAP_FAILED) {
        ...
        auto _close_fd = lockbox::create_deferred(close, shm_fd);
    ...
    bsd_name() {
        ...
        auto _free_disk_service = lockbox::create_deferred(IOObjectRelease, disk_service);
        
        CFTypeRef prop = IORegistryEntryCreateCFProperty(disk_service, (CFStringRef) @"BSD Name", kCFAllocatorDefault, 0);
        auto _free_prop = lockbox::create_deferred(CFRelease, prop);
...
get_ram_disk_bsd_name() {
    ...
    auto _close_open = lockbox::create_deferred(pclose, f);
