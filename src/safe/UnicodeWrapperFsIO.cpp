...
public:
  ...
  virtual opt::optional<encfs::FsDirEnt> readdir() override {
    ...
    while (!toret) {
      ...
      if (!safe::unicode_fs::is_normalized_path_component(toret->name)) {
        toret = opt::nullopt;
      }
    }
    toret->name = safe::unicode_fs::normalize_path_component_for_user(toret->name);
...
UnicodeWrapperFsIO::_convertPath(const encfs::Path & path) const {
  ...
  for (const auto & p : comps) {
    // normalized p
    auto nfc_p = safe::unicode_fs::normalize_path_component_for_fs(p);
    ...
    if (maybe_dir) {
      ...
      while ((ent = maybe_dir->readdir())) {
        if (!safe::unicode_fs::is_normalized_path_component(ent->name)) {
          // skip bad path
          continue;
        }
        if (safe::unicode_fs::normalized_path_components_equal(ent->name, nfc_p)) {
...
UnicodeWrapperFsIO::opendir(const encfs::Path & path) const {
  return safe::make_unique<UnicodeWrapperDirectoryIO>(_base->opendir(_convertPath(path)));
}
