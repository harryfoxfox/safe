...
#ifndef __safe_windows_async_hpp
#define __safe_windows_async_hpp
...
modal_until_message(HWND parent, std::string title, std::string ui_msg,
  ...
  auto _destroy_dialog_wnd = safe::create_destroyer(dialog_wnd, DestroyWindow);
...
modal_until_object(HWND parent, std::string title, std::string ui_msg,
  ...
  // disable window
  decltype(safe::create_deferred(EnableWindow, parent, TRUE)) _renable;
  if (parent) {
    EnableWindow(parent, FALSE);
    _renable = safe::create_deferred(EnableWindow, parent, TRUE);
  }
...
run_async_base(FunctionB && loop_stop,
  ...
  auto _close_thread = safe::create_destroyer(ret_thread, CloseHandle);
