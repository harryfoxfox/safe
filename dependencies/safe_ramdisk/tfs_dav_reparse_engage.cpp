...
query_value_key_allocate(HANDLE KeyHandle,
  ...
  *KeyValueInformation = ExAllocatePoolWithTag(PoolType, value_length,
  ...
  auto _free_key_value_information =
    safe::create_deferred(ExFreePool, *KeyValueInformation);
...
get_path_to_tfs_dav(PUNICODE_STRING path_to_tfs_dav) {
  ...
  auto _close_key =
    safe::create_deferred(ZwClose, windows_reg_key);
  ...
  auto _free_key_value =
    safe::create_deferred(ExFreePool, key_value);
...
does_tfs_dav_link_already_exists(PUNICODE_STRING path_to_tfs_dav,
  ...
  *reparse_handle = nullptr;
  ...
  auto _close_file = safe::create_deferred(ZwClose, existing_tfs_dav_handle);
...
rename_tfs_dav_directory(PUNICODE_STRING path_to_tfs_dav, RenameDirection dir) {
  ...
  auto _free_new_tfs_dav_path =
    safe::create_deferred(free_unicode_string, &new_tfs_dav_path);
...
create_new_tfs_dav_link(PUNICODE_STRING path_to_tfs_dav,
  ...
  auto _close_file = safe::create_deferred(ZwClose, tfs_dav_handle);
...
engage_reparse_point(PHANDLE reparse_handle) {
  ...
  auto _free_tfs_path =
    safe::create_deferred(free_unicode_string, &path_to_tfs_dav);
...
iterate_over_files(PUNICODE_STRING path, F handle_entry) {
  ...
  auto _free_info =
    safe::create_deferred(ExFreePoolWithTag,
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (status2 == STATUS_BUFFER_OVERFLOW) {
        ...
        _free_info = 
          safe::create_deferred(ExFreePoolWithTag, info,
                                   DELETE_TFS_DAV_CHILDREN_TAG);
      }
      else break;
    }
