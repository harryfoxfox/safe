...
RAMDiskDevice::create_remove_lock_guard(RAMDiskDevice::RemoveLockGuard *out) {
  ...
  *out = safe::create_deferred(_free_remove_lock, this);
...
RAMDiskDevice::dequeue_request(PIRP *out,
  ...
  while (true) {
	...
	&this->terminate_thread,
      ...
      auto status =
	KeWaitForMultipleObjects(safe::numelementsf(wait_objects),
				 (PVOID *) wait_objects,
				 WaitAny,
				 Executive,
				 KernelMode,
...
RAMDiskDevice::worker_thread() noexcept {
  ...
  while (true) {
    ...
    auto _free_lock =
      safe::create_deferred(_free_remove_lock, this);
...
create_ramdisk_device(PDRIVER_OBJECT driver_object,
  ...
  auto _delete_device_object =
    safe::create_deferred(delete_ramdisk_device, device_object);
...
read_write_section(HANDLE section, size_t offset,
  ...
  auto _unmap_view =
    safe::create_deferred(ZwUnmapViewOfSection,
...
read_registry_alloc_size(PDEVICE_OBJECT pdo,
  ...
  auto _close_key = safe::create_deferred(ZwClose, reg_key);
